...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public sealed class Character : CharacterBehaviour
    {
        ...
        [Tooltip("Перетягни сюди Global Volume зі сцени")]
        [SerializeField] private Volume globalVolume;

        [Tooltip("Інтенсивність віньєтки, коли гравець присів")]
        [SerializeField] private float crouchVignetteIntensity = 0.35f;

        [Header("Noise Settings")]
        [SerializeField] private float runNoiseRadius = 11f;
        ...
        private bool holdingButtonJump; // Чи тримаємо пробіл
        ...
        {
            if (GetInputMovement().sqrMagnitude < 0.1f) return;
        ...
        private void UpdateAnimator()
        {
            ...
            characterAnimator.SetFloat(HashMovement, inputMagnitude, dampTimeLocomotion, Time.deltaTime);
        ...
        /// </summary>
        public void SetHolstered(bool value = true)
        {
            //Update value.
            holstered = value;
        ...
        public void PlayLandingMotion(float verticalIntensity)
        {
            if (landingCoroutine != null) StopCoroutine(landingCoroutine);
            landingCoroutine = StartCoroutine(LandingDropCoroutine(verticalIntensity));
        }
        ...
        public void OnTryPlayReload(InputAction.CallbackContext context)
        {
            ...
            {//Performed.
                case { phase: InputActionPhase.Performed }:
                    ...
                    // --- НОВА ПЕРЕВІРКА ПАТРОНІВ ---
                    // 1. Отримуємо посилання на твій скрипт Weapon
                    Weapon customWeapon = equippedWeapon as Weapon;

                    // 2. Якщо зброя існує І вона каже "Я не можу перезарядитись" (бо немає патронів в інвентарі)
                    if (customWeapon != null && !customWeapon.CanReload())
                    {
                        // Тут можна додати звук "клац" (пустий), якщо хочеш
                        // if(customWeapon.GetAudioClipFireEmpty()) ...

                        return; // <--- СТОП! Ми виходимо з функції і не запускаємо анімацію.
                    }
                    // -------------------------------
